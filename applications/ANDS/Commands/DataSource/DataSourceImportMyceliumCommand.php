...
class DataSourceImportMyceliumCommand extends ANDSCommand
{
    protected function configure()
    {
        $this
            ->setName('ds:import-mycelium')
            ->setDescription('Import a datasource to mycelium')
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $this->log("Updated DataSource[id={$dataSource->id}] in Mycelium");

        $batchID = uniqid();
        $result = $client->createNewImportRecordRequest($batchID);
        $request = json_decode($result->getBody()->getContents(), true);
        $myceliumRequestId = $request['id'];
        $this->log("MyceliumRequestId: $myceliumRequestId");
        $this->log("BatchID: $batchID");
        ...
        foreach ($records as $id) {
            ...
            $client->importRecord($record, $myceliumRequestId);
            $progressBar->advance();
        }
    }
}