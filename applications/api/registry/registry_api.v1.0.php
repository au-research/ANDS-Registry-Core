...
//using the default Exception class
...
class Registry_api
{
    ...
    public function handle($method = array())
    {
        ...
        //Accepted handler will be run here
        if ($this->params['submodule'] && method_exists($this, $this->params['submodule'])) {
            $handler = $this->params['submodule'];
            return $this->$handler();
        } else {
    ...
    private function search_api()
    {
        ...
        $forwarded_params = array_intersect_key(array_flip($this->valid_filters), $this->ci->input->get());
