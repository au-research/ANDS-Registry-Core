 ...
 */
 ;(function($) {
	...
	$.fn.registry_widget = function(options, param) {
      ...
      proxy: 'https://researchdata.edu.au/api/v2.0/registry.jsonp/',
	...
	function _search(obj, s){
		...
		var value = obj.val();
		$.ajax({
			url:s.proxy+'search/?api_key='+s.api_key+'&q='+obj.val()+'&callback=?',
			dataType:'jsonp',
			timeOut:5000,
	...
	function _lookup(query, obj,s){
		$.ajax({
			url:s.proxy+'lookup/?api_key='+ s.api_key+'&q='+encodeURIComponent(query)+'&callback=?',
			dataType:'jsonp',
			timeOut:5000,
			success: function(data){
				if(data.status==0){
            ...
            error: function(data){
                console.log('function_error',data);
            }
		});
	}
