<?php
    ...
    $relatedTypes = array('collection', 'party', 'activity', 'service', 'publication','website');
	if ($ro->relatedInfo) {
        ...
        foreach ($ro->relatedInfo as $relatedInfo) {
           if (!in_array($relatedInfo['type'],$notTypes)) {
                if(in_array($relatedInfo['type'],$relatedTypes) && $relatedInfo['title'] != ""  && !$has_moreInfo){
                    $has_moreInfo = false;
                }
                elseif($relatedInfo['type'] == 'service' && ($relatedInfo['relation']['url']== '' || $relatedInfo['title'] != '') && !$has_moreInfo)
                {
                    $has_moreInfo = false;
                }
	...
	<div class="panel">
		...
		<div class="panel-body swatch-white">
           ...
           @elseif(!in_array($relatedInfo['type'],$notTypes) && isset($relatedInfo['identifier']['identifier_value']) && !in_array(trim($relatedInfo['identifier']['identifier_value']), $resolvedPartyIdentifiers) )
                @if(!in_array($relatedInfo['type'],$relatedTypes) || (in_array($relatedInfo['type'],$relatedTypes) && $relatedInfo['title']==''))
