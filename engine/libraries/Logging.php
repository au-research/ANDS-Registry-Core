...
class Logging
{
	...
	/**
	 * Adding a logger object to the array
	 * this enable modular level logging and allow the logging interface not to be
	 * defined in the logging.php config file, but can be defined elsewhere
	 * @author Minh Duc Nguyen <minh.nguyen@ands.org.au>
	 * @param php_array $logger in the form of
	 * (
	 *     'type' => <class_name>,
	 *     'name' => <name>
	 *     'level' => <level>,
	 *     'format' => <format>
	 *     <other_parameters as required>
	 * )
	 */
...
/**
 * Logging that will log the entry into a Database table
 * Requires the database, table
 */
class DatabaseLogger extends Logger {
	...
	public function initialise($params) {
		parent::initialise($params);
		$this->database_group = $params['database_group'];
		$this->table = $params['table'];
		$this->enabled = TRUE;
	}

	protected function do_log($log_entry) {
		$db = $this->CI->load->database($this->database_group, TRUE);
		if (!$db) throw new LoggingException('The database group '. $this->database_group. ' is not defined correctly');
		$log = array(
			'class' => 'vocab',
			'type' => 'activity',
			'message' => $log_entry
		);
		$db->insert($this->table, $log);
	}
