...
class Solr
{
    ...
    /**
     * Get the current SOLR URL
     * @return mixed
     */
    public function getSolrUrl()
    {
        return $this->solr_url . $this->getCore(). '/';
    }
    ...
    /**
     * Should work for both http://localhost:8983/solr/ and http://localhost:8983/solr/portal/
     * Cater for configuration value that includes the core name, like http://localhost:8983/solr/portal/
    ...
    private function fixSolrUrl(){
       // dd($this->solr_url);
        ...
        if (end($chunks) != "solr") {
            $this->setCore(end($chunks));
            array_pop($chunks);
        }
        $this->solr_url = implode('/', $chunks).'/';
    }
...
//                    $this->setOpt('fq', "+(earliest_year:[$from TO *] AND latest_year:[* TO $to]) OR (earliest_year:[$from TO *] AND earliest_year:[* TO $to]) OR (latest_year:[* TO $to] AND latest_year:[$from TO *])");
    ...
    */
    ...
    function post($docs, $handle = 'select')
    {
        ...
        curl_setopt($ch, CURLOPT_URL, $this->getSolrUrl() . $handle);//post to SOLR
    ...
    {
        return curl_post($this->getSolrUrl() . 'update/?wt=json&commit=true', $docs, array("Content-Type: application/json; charset=utf-8"));
    }

    function commit()
    {
        return curl_post($this->getSolrUrl() . 'update?wt=json&commit=true', '<commit waitSearcher="false"/>');
    }

    function deleteByQueryCondition($query_condition)
    {
        if ($query_condition) {
            $result = curl_post($this->getSolrUrl() . 'update?commit=true&wt=json',
    ...
    function clear($data_source_id = 'all')
    {
        ...
        $result = curl_post($this->getSolrUrl() . 'update?commit=true&wt=json', '<delete><query>' . $query . '</query></delete>');
